...
input_names = ["prompt_token", "prompt_token_len", "speech_token", "speech_token_len"]
output_names = ["mu", "mask", "prompt_mel_len", "output_mel_len"]
...
class TokenToMuExport(torch.nn.Module):
    ...
    def forward(self, prompt_token, prompt_token_len, speech_token, speech_token_len):
        ...
        return mu, mask, prompt_mel_len, output_mel_len


def make_dummy_inputs(batch: int = 1, prompt_tokens: int = 16, speech_tokens: int = 32):
    return (
